...
QUnit.test(
    ...
    function (assert)
    {
        ...
        // dates in the past
        assert.equal(formatRelative(new Date(2017, 11, 31, 11, 59, 50), referenceDate), "in diesem Moment", "10s ago");
        assert.equal(formatRelative(new Date(2017, 11, 31, 11, 59, 13), referenceDate), "in diesem Moment", "border of seconds-minutes slip: seconds, past");
        ...
        assert.equal(formatRelative(referenceDate, new Date(2017, 11, 31, 11, 59, 50)), "in diesem Moment", "in 10s");
        assert.equal(formatRelative(referenceDate, new Date(2017, 11, 31, 11, 59, 13)), "in diesem Moment", "border of seconds-minutes slip: seconds, future");
        ...
        // same date
        assert.equal(formatRelative(referenceDate, referenceDate), "in diesem Moment", "same date");
        ...
        assert.equal(formatRelative(new Date()), "in diesem Moment", "default reference date");
    }
);
